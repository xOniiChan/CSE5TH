...
int main() {
    ...
    process p[n]; // Variable length array (VLA), not standard in C++ but works in GCC/Clang. For standard C++, use std::vector<process>.

    // Input the process details
    for (int i = 0; i < n; i++) {
        cout << "Enter details for process " << i + 1 << ":\n";
        p[i].pid = i + 1; // Assuming PID is assigned sequentially
